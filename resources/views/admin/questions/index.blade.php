...
@section('content')
    <div class="max-w-7xl mx-auto animate-fadeIn">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('admin.duplicates') }}"
                    class="px-5 py-2.5 bg-rose-100 hover:bg-rose-200 text-rose-700 font-semibold rounded-xl transition-all inline-flex items-center space-x-2">
                ...
                <a href="{{ route('admin.questions.create') }}"
                    class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-emerald-500/30 transition-all transform hover:-translate-y-0.5 inline-flex items-center space-x-2">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <form method="GET" action="{{ route('admin.questions.index') }}"
            class="mb-6 bg-white/70 backdrop-blur-xl border border-slate-200 rounded-2xl p-4 flex flex-wrap gap-3 items-end">
            <div class="space-y-1">
            ...
            <div class="space-y-1">
                ...
                <select name="difficulty"
                    class="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-medium">
            ...
            <div class="space-y-1">
                ...
                <select name="theme_id"
                    ...
                    @foreach($themes as $t)
                        <option value="{{ $t->id }}" {{ request('theme_id') == $t->id ? 'selected' : '' }}>{{ $t->name }}
                            ({{ $t->type }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <button type="submit"
                class="px-4 py-2 bg-slate-800 text-white rounded-lg text-sm font-bold hover:bg-slate-700 transition-colors">Filter</button>
            <a href="{{ route('admin.questions.index') }}"
                class="px-4 py-2 bg-slate-100 text-slate-600 rounded-lg text-sm font-bold hover:bg-slate-200 transition-colors">Clear</a>
        </form>
        ...
        <div class="bg-white/70 backdrop-blur-xl border border-slate-200 shadow-xl rounded-2xl overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead
                        class="bg-slate-50/80 border-b border-slate-200 text-slate-600 uppercase text-xs font-bold tracking-wider">
                        <tr>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($questions as $q)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-4 py-4">
                                    <span
                                        class="px-2.5 py-1 text-xs font-bold rounded-lg {{ $q->type === 'PARA' ? 'bg-sky-100 text-sky-700' : 'bg-amber-100 text-amber-700' }}">
                                        {{ $q->type }}
                                    </span>
                                </td>
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-12 text-center text-slate-500">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-xs text-slate-400 mt-1">Add your first manual question to get started.
                                        </p>
                                    </div>
                                </td>
                            </tr>
