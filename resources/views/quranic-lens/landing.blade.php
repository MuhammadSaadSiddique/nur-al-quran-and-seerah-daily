...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Amiri:wght@400;700&family=Inter:wght@300;400;500;600;700;800;900&family=Scheherazade+New:wght@400;700&display=swap"
        rel="stylesheet">
...
<body class="bg-slate-900 text-slate-100 antialiased selection:bg-emerald-500 selection:text-white">
    ...
    <header class="relative pt-32 pb-24 flex items-center justify-center overflow-hidden min-h-screen">
        <div
            class="absolute inset-0 z-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-emerald-950/40 via-slate-900 to-slate-900">
        </div>
        <div class="relative z-10 max-w-5xl mx-auto px-4 text-center space-y-8">
            <div
                class="inline-flex items-center space-x-2 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider animate-pulse">
                🔬 Advanced Quranic Research & Analytics
            </div>
            <h1 class="text-5xl md:text-7xl font-extrabold tracking-tight text-white leading-tight">
                Analytical Connection Mapping of the <span
                    class="bg-gradient-to-r from-emerald-400 to-teal-300 bg-clip-text text-transparent">Holy
                    Quran</span>
            </h1>
            <p class="text-lg md:text-xl text-slate-300 max-w-3xl mx-auto font-light leading-relaxed">
                Welcome to a collaborative research ecosystem documenting the harmony between Divine Revelation and
                human observations. Search and explore connections mapping verses to Science, Seerah, Hadith, History,
                and Scriptures.
            </p>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-6 gap-8 text-center">
                ...
                <div class="space-y-2 col-span-2 md:col-span-1">
                    <p class="text-4xl font-extrabold text-white">{{ max($stats['researchers'], 50) }}+</p>
                    <p class="text-xs uppercase font-semibold text-slate-400 tracking-wider"> Researchers</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16" x-data="{ activeTab: 'science' }">
        <div class="text-center space-y-4">
            ...
            <p class="text-slate-400 max-w-2xl mx-auto text-base">
                Our analytical engine structures research into five primary context lenses, linking revelation to
                historical and observable reality.
            </p>
        </div>
        ...
        <div class="flex flex-wrap justify-center gap-2 border-b border-white/10 pb-4">
            <button @click="activeTab = 'science'"
                :class="activeTab === 'science' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'"
                class="px-5 py-2.5 rounded-xl font-bold transition-all text-sm">
                🔬 Science & Cosmology
            </button>
            <button @click="activeTab = 'seerah'"
                :class="activeTab === 'seerah' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'"
                class="px-5 py-2.5 rounded-xl font-bold transition-all text-sm">
                🕌 Seerah Chronology
            </button>
            <button @click="activeTab = 'hadith'"
                :class="activeTab === 'hadith' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'"
                class="px-5 py-2.5 rounded-xl font-bold transition-all text-sm">
                📜 Hadith Exegesis
            </button>
            <button @click="activeTab = 'history'"
                :class="activeTab === 'history' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'"
                class="px-5 py-2.5 rounded-xl font-bold transition-all text-sm">
                🏛️ History & Archeology
            </button>
            <button @click="activeTab = 'scripture'"
                :class="activeTab === 'scripture' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'"
                class="px-5 py-2.5 rounded-xl font-bold transition-all text-sm">
        ...
        <!-- Tab Panels -->
        <div
            class="bg-slate-800/50 border border-white/5 p-8 rounded-3xl backdrop-blur-sm min-h-[300px] flex flex-col justify-between">
            ...
            <div x-show="activeTab === 'science'" x-cloak class="space-y-6">
                <div class="space-y-2">
                    ...
                    <p class="text-slate-300 leading-relaxed">
                        Explores the correlations between statements in the Quran and modern scientific discoveries
                        across geology, embryology, astronomy, hydrology, and physics. The platform logs each
                        observation with comprehensive peer-reviewed validations.
                    </p>
                </div>
                <div class="grid md:grid-cols-2 gap-4 pt-4">
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        ...
                        <p class="text-sm text-slate-400 mt-2">Meticulous tracing of planetary motion, orbits, and
                            expanding universe theories linked directly to Surah Ad-Dhariyat and Al-Anbya.</p>
                    </div>
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        ...
                        <p class="text-sm text-slate-400 mt-2">Tracing stages of embryological development mentioned in
                            Surah Al-Mu'minun with physiological correlations.</p>
                    </div>
                </div>
            </div>
            ...
            <div x-show="activeTab === 'seerah'" x-cloak class="space-y-6">
                <div class="space-y-2">
                    ...
                    <p class="text-slate-300 leading-relaxed">
                        Every verse has a context (Asbab al-Nuzul). By linking revelation events to specific timestamps
                        and geographic stages in the Seerah of Prophet Muhammad ﷺ, we establish a chronological timeline
                        of revelation.
                    </p>
                </div>
                <div class="grid md:grid-cols-2 gap-4 pt-4">
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        ...
                        <p class="text-sm text-slate-400 mt-2">Chronological linking of early Meccan Surahs detailing
                            patience, creation proofs, and stories of past prophets to strengthen the Sahabah.</p>
                    </div>
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        ...
                        <p class="text-sm text-slate-400 mt-2">Mapping Medinan verses detailing legal jurisprudence,
                            civic treaties, and historical battles directly to the Seerah milestones.</p>
                    </div>
                </div>
            </div>
            ...
            <div x-show="activeTab === 'hadith'" x-cloak class="space-y-6">
                <div class="space-y-2">
                    ...
                    <p class="text-slate-300 leading-relaxed">
                        Hadith is the key to unlocking the detailed application of the Quranic text. This database
                        builds semantic connections between verses and authentic ahadith from Sahih al-Bukhari, Sahih
                        Muslim, and other prime collections.
                    </p>
                </div>
                <div class="grid md:grid-cols-2 gap-4 pt-4">
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        <span class="text-xs uppercase font-bold text-emerald-400 tracking-wide">Hadith
                            Correlation</span>
                        <h4 class="font-bold text-white mt-1">Practical Jurisprudence</h4>
                        <p class="text-sm text-slate-400 mt-2">Mapping short, summary statements of worship commands in
                            the Quran to their step-by-step practical guides in the Sunnah.</p>
                    </div>
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        ...
                        <p class="text-sm text-slate-400 mt-2">Linking prophetic descriptions of moral excellence to
                            ethical demands in the verses.</p>
                    </div>
                </div>
            </div>
            ...
            <div x-show="activeTab === 'history'" x-cloak class="space-y-6">
                <div class="space-y-2">
                    ...
                    <p class="text-slate-300 leading-relaxed">
                        Quranic accounts describe ancient civilizations, prophets, and cultures (e.g. Ad, Thamud,
                        Pharaoh, Byzantine Empire). Our platform maps historical records, clay tablets, and
                        archeological findings that back these narratives.
                    </p>
                </div>
                <div class="grid md:grid-cols-2 gap-4 pt-4">
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        <span class="text-xs uppercase font-bold text-emerald-400 tracking-wide">Historical
                            Reference</span>
                        <h4 class="font-bold text-white mt-1">The Byzantine Victory</h4>
                        <p class="text-sm text-slate-400 mt-2">Correlating Surah Ar-Rum's prediction of Roman victory
                            after defeat with exact Roman-Sasanian battle dates.</p>
                    </div>
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        <span class="text-xs uppercase font-bold text-teal-400 tracking-wide">Archaeological
                            Findings</span>
                        <h4 class="font-bold text-white mt-1">Pharaonic Preservation</h4>
                        <p class="text-sm text-slate-400 mt-2">Verifying historical details of ancient Egyptian rulers
                            and mummification records as referenced in Surah Yunus.</p>
                    </div>
                </div>
            </div>
            ...
            <div x-show="activeTab === 'scripture'" x-cloak class="space-y-6">
                <div class="space-y-2">
                    ...
                    <p class="text-slate-300 leading-relaxed">
                        The Quran affirms it was sent to confirm and correct previous revelations (Torah, Gospel,
                        Psalms). This database tracks theological correlations, similarities, and corrections between
                        Quranic verses and biblical verses.
                    </p>
                </div>
                <div class="grid md:grid-cols-2 gap-4 pt-4">
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        <span class="text-xs uppercase font-bold text-emerald-400 tracking-wide">Comparative
                            Theology</span>
                        <h4 class="font-bold text-white mt-1">The Abrahamic Creed</h4>
                        <p class="text-sm text-slate-400 mt-2">Mapping monotheistic statements in Surah Al-Ikhlas to
                            biblical references in Deuteronomy and Isaiah.</p>
                    </div>
                    <div class="bg-slate-900/60 p-5 rounded-2xl border border-white/5">
                        ...
                        <p class="text-sm text-slate-400 mt-2">Cross-referencing events from the life of Prophet Joseph
                            (Yusuf) in Surah Yusuf with Genesis.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
                ...
                <a href="{{ route('welcome') }}"
                    class="text-emerald-400 hover:text-emerald-300 font-bold flex items-center gap-1 transition-all">
    ...
    <section class="py-20 bg-slate-950/60 border-t border-white/5">
        <div class="max-w-5xl mx-auto px-4 space-y-12">
            <h2 class="text-3xl md:text-4xl font-extrabold text-white text-center">Why Document Quranic Connections?
            </h2>

            <div class="grid md:grid-cols-3 gap-8">
                <div class="space-y-3">
                    <div
                        class="w-12 h-12 bg-emerald-600/10 border border-emerald-500/20 text-emerald-400 rounded-2xl flex items-center justify-center font-bold text-xl">
                        1</div>
                    <h3 class="text-lg font-bold text-white">Empirical Harmony</h3>
                    <p class="text-sm text-slate-400 leading-relaxed">
                        Rather than separating science and theology, mapping connections helps demonstrate how physical
                        reality conforms to structural descriptions in revelation.
                    </p>
                </div>
                <div class="space-y-3">
                    <div
                        class="w-12 h-12 bg-teal-600/10 border border-teal-500/20 text-teal-400 rounded-2xl flex items-center justify-center font-bold text-xl">
                        2</div>
                    <h3 class="text-lg font-bold text-white">Intertextual Clarity</h3>
                    <p class="text-sm text-slate-400 leading-relaxed">
                        Tracing comparative references across older Judeo-Christian scriptures establishes the
                        continuity of the Abrahamic monotheistic lineage.
                    </p>
                </div>
                <div class="space-y-3">
                    <div
                    ...
                    <p class="text-sm text-slate-400 leading-relaxed">
                        All claims of connection undergo rigorous review by verified research peers, preventing
                        arbitrary claims and preserving semantic integrity.
                    </p>
                </div>
            </div>
        </div>
    ...
    <section class="py-24 bg-gradient-to-b from-slate-900 to-slate-950 border-t border-white/5 relative">
        <div
            class="absolute inset-0 z-0 bg-[radial-gradient(circle_at_bottom,_var(--tw-gradient-stops))] from-emerald-950/20 via-transparent to-transparent">
        </div>
        <div class="relative z-10 max-w-4xl mx-auto px-4 text-center space-y-8">
            ...
            <p class="text-slate-300 max-w-2xl mx-auto text-base">
                Are you an expert in Islamic theology, comparative scriptures, archaeology, or modern scientific fields?
                Contribute to our growing connection matrix.
            </p>
            <div class="pt-4 flex flex-col sm:flex-row justify-center gap-4">
                @auth
                    @if(auth()->user()->is_researcher)
                        <a href="{{ route('admin.lens.approvals.index') }}"
                            class="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-full transition-all">
                    ...
                    @else
                        <form action="{{ route('researchers.join') }}" method="POST" class="inline">
                            @csrf
                            <button type="submit"
                                class="w-full sm:w-auto px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-full shadow-lg transition-all">
                                Join Research Team
                            </button>
                        </form>
                    @endif
                @else
                    <a href="{{ route('login') }}"
                        class="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-full shadow-lg transition-all">
    ...
    <footer class="bg-slate-950 text-slate-500 py-16 text-center text-sm border-t border-white/5">
        <div class="max-w-7xl mx-auto px-4 space-y-6">
            ...
            <p class="opacity-60 max-w-md mx-auto">
                An advanced Quranic research database cataloging comparative theology and scientific concordance
                mapping.
            </p>
